...
go mod tidy
mkdir -p output
GOOS=linux GOARCH=arm64 go build -v -o output/arm64 -ldflags "-w -s" ./internal/cmd/agent
GOOS=linux GOARCH=arm go build -v -o output/arm -ldflags "-w -s" ./internal/cmd/agent
GOOS=linux GOARCH=amd64 go build -v -o output/x64 -ldflags "-w -s" ./internal/cmd/agent
popd

mkdir -p module/beszel/bin
mv beszel/output/* module/beszel/bin/
